...
Creates the Resnet model as per the arguments passed.

Arguments:
1. `depth`: The depth of the Resnet Model. The current allowed
            values are `18`, `34`, `50`, `101` and `152`.
2. `pretrained`: Gets the network with pretrained Imagenet weights.
                 `Metalhead.jl` currently has only the weights for
                 ResNet 50 so sending `depth` anything other than
                 `50` results in an **ERROR**.
"""

function get_resnet_model(depth, pretrained::Bool = false)
  if pretrained && depth != 50
    error("Pretrained weights are only available for ResNet 50")
  end
  if depth==18
    _ResNet(BasicBlock, [2, 2, 2, 2]) |> gpu
  elseif depth==34
    _ResNet(BasicBlock, [3, 4, 6, 3]) |> gpu
  elseif depth==50
    if pretrained
      m = ResNet() |> gpu
      ...
      _ResNet(Bottleneck, [3, 4, 6, 3]) |> gpu
    end
  elseif depth==101
    _ResNet(Bottleneck, [3, 4, 23, 3]) |> gpu
  elseif depth==152
    _ResNet(Bottleneck, [3, 8, 36, 3]) |> gpu
  end
end
